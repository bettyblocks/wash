...
const USER_COMPONENT: &[u8] = include_bytes!("fixtures/user_component.wasm");
const ADMIN_COMPONENT: &[u8] = include_bytes!("fixtures/admin_component.wasm");

async fn start_path_router_host() -> Result<(impl HostApi, SocketAddr)> {
    let engine = Engine::builder().build()?;
    let port = find_available_port().await?;
    let addr: SocketAddr = format!("127.0.0.1:{port}").parse().unwrap();
    let http_plugin = HttpServer::new(PathRouter::default(), addr);
